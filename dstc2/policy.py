...
'''
  This script is for policy (dialog act) prediction.
...
class Parameters():
    batch_size = 4000
    test_size = 11237  # size of test set
    train_size = 14404
    grad_clip = 1
    epoch_num = 200
    learning_rate = 0.1
    ...
    data_dir = '/home/qihu/PycharmProjects/e2e-dm_babi/data/data'
    tmp_dir = '/home/qihu/PycharmProjects/e2e-dm_babi/tmp/policy_dstc2'
    vocab_path = os.path.join(data_dir, 'all_vocab.txt')
    kb_path = os.path.join(data_dir, 'dialog-babi-task6-data-kb.txt')
    template_path = os.path.join(data_dir, 'sys_resp_template.labeled.txt')
    # Train/Dev/Test path
...
class Data(object):
    def __init__(self, params):
        ...
        self.id2act = ['hello', 'api', 'recommend', 'confirm_intent', 'you_are_welcome', 'request_food',
                       'request_area', 'no_result', 'inform_address', 'inform_phone', 'inform_food', 'inform_price',
                       'offer_help', 'request_price', 'inform_post', 'confirm_food', 'confirm_price', 'confirm_area']
...
class Policy(object):
    def __init__(self, params):
        # Input variable
        if infer == 1:
            params.batch_size = params.train_size
...
def train(data, model, params):
    ...
    m = 11
    dev_sys = dl.flatten_2D(data.dev_sys)
    print 'USER WORDS', data.dev_input_id[m]
    print 'API RESULT', dev_api[m]
    print 'SYS WORDS', dev_sys[m]
    print 'SYS ACT', dev_act[m]
...
def test_on_trainset(data, model, params):
    ...
    print len(data.train_input_id), len(train_api), len(train_act)
    ...
    true_list = np.asarray(data.train_label) - 1
    pred_type_list = []
    true_type_list = []
    for i in range(params.train_size):
        ...
        act_id = np.argmax(p)
        string = '%s %s\t#%s\t#%s\t%d\n' % (
        data.id2act[act_id], data.id2act[data.train_label[i] - 1], train_usr[i], train_sys[i], data.train_api_num[i])
        if act_id == data.train_label[i] - 1:
            f_true.write(string)
        else:
            error += 1
        ...
        if data.id2act[data.train_label[i] - 1] not in true_type_list:
            true_type_list.append(data.id2act[data.train_label[i] - 1])
        pred_list[i] = act_id
        # print error
...
def test(data, model, params):
    ...
    error = 0
    test_api = data.get_api_vector(data.test_api_num)
    test_act = data.get_act_vector(data.test_label)
    test_feed_dict = {
        model.x_word: data.test_input_id,
        model.x_api: test_api,
        model.y_act: test_act,
        model.dropout_keep: 1.0
    }
    print len(data.test_input_id), len(test_api), len(test_act)
    prob = sess.run(model.prob, feed_dict=test_feed_dict)
    test_usr = dl.flatten_2D(data.test_usr)
    test_sys = dl.flatten_2D(data.test_sys)
    pred_list = np.zeros(params.test_size)
    true_list = np.asarray(data.test_label)-1
    pred_type_list = []
    true_type_list = []
    for i in range(params.test_size):
        p = prob[i]
        act_id = np.argmax(p)
        string = '%s %s\t#%s\t#%s\t%d\n' % (data.id2act[act_id], data.id2act[data.test_label[i]-1], test_usr[i], test_sys[i], data.test_api_num[i])
        if act_id == data.test_label[i]-1:
            f_true.write(string)
        else:
            error += 1
            f_false.write(string)
        if data.id2act[act_id] not in pred_type_list:
            pred_type_list.append(data.id2act[act_id])
        if data.id2act[data.test_label[i]-1]not in true_type_list:
            true_type_list.append(data.id2act[data.test_label[i]-1])
        pred_list[i] = act_id
        # print error
    print 'Pred type: ', pred_type_list
    print 'True type; ', true_type_list
    print 'Error rate: %.3f' % (error*1.0/params.test_size)
    cm = confusion_matrix(true_list, pred_list)
...
def main(infer, restore=False):
    ...
    if infer:
        print('Testing')
        test_on_trainset(data, model, params)
    else:
        print('Training')
